...
class MediaQueueModel(QAbstractListModel):
        ...
        self._items.append(
            {
                ...
                "detail": "Reading streams, codecs and duration",
                "kind": "unknown",
                "duration": "—",
    ...
    def mark_processing(self, job_id: UUID) -> None:
        self._update(
            ...
            detail="FFmpeg is processing this file in the background",
    ...
    def mark_cancelled(self, job_id: UUID) -> None:
        self._update(
            ...
            detail="No output was published",
            progress=0.0,
    ...
    def clear_finished(self) -> None:
        ...
        self._assets = {job_id: asset for job_id, asset in self._assets.items() if job_id in retained_ids}
        self.endResetModel()
        self.countChanged.emit()
        self.summaryChanged.emit()
